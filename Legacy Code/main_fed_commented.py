...
def process(args):
    ...
    if args.model == 'cnn' and args.dataset == 'MNIST':
        ...
        len_in = 1
        img_size = dataset_train.dataset[0][0].shape  # Get shape of input tensor
        for dim in img_size:
            len_in *= dim  # Calculate total input dimension
        net_glob = MLP(dim_in=len_in, dim_hidden=200, dim_out=args.num_classes).to(args.device)
    else:
        exit('Error: unrecognized model')
    ...
    for i in range(duplication):
        ...
        for iter in range(args.epochs):
            ...
            start_time = time.time()
            for idx in idxs_users:
                local = LocalUpdate(args=args, dataset=dataset_train.dataset, idxs=dict_party_user[idx])
                w, loss = local.train(net=copy.deepcopy(net_glob).to(args.device))
                if args.all_clients:
                    w_locals[idx] = copy.deepcopy(w)
                else:
